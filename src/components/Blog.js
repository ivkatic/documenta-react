...
class Blog extends Component {
    ...
    getArticles() {
        ...
        api.get(`posts?per_page=${this.state.per_page}${requestLocale}`, null, true ).then(result => {
            if(result && result != '') { 
                this.setState(state => ({
                    btn_class: 'fa fa-plus-circle',
                    posts: result.posts,
        ...
        if(this.state.posts != null) {
            return (
                <main id="content" className="container mx-auto mt-12 mb-24">
                    <div className="mb-12 w-full text-3xl"><h4>Blog</h4></div>
                    <div className="md:flex fp-news">
                        ...
                        <div className="md:w-3/12 md:pl-6">
                            <Sidebar type="blog_posts" postId="" content={this.state.sidebar} />
                        </div>
                    </div>
                </main>
            );
